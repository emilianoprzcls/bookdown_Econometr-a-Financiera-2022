...
Es facil ver que muchas de las posibilidades de realización de este proceso estocástico (series de tiempo) pueden tomar cualquiera de las rutas consideradas en el Figura \ref{fig:Caminata10}. Ahora analicemos un solo camino.
...
\NormalTok{caminata\_ts}\OtherTok{\textless{}{-}}\FunctionTok{ts}\NormalTok{(G\_t1,}\AttributeTok{star=}\DecValTok{1}\NormalTok{,}\AttributeTok{end=}\NormalTok{Soporte)}
\end{Highlighting}
\end{Shaded}
...
Como se comentó con anterioridad en la Figura \ref{fig:ACFCAMINATA1} es evidente que la Caminata si tiene autocorrelacion, por lo que nuestro plot de autocorrelacion tiene valores muy altos en todos los lags. Veamos los lags.
...
De nuevo, esto al ser creado de manera estandarizada estamos seguros de que va a ser estacionario en la medio, por lo mismo los lags de la Figura \ref{fig:LAGSCAMINATA1} se ven tan correlacionados.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{MA\_m5}\OtherTok{\textless{}{-}}\NormalTok{forecast}\SpecialCharTok{::}\FunctionTok{ma}\NormalTok{(ret\_amazn\_ts,}\AttributeTok{order=}\DecValTok{11}\NormalTok{,}\AttributeTok{centre=}\ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(ret\_amazn\_ts)}\SpecialCharTok{+}\FunctionTok{lines}\NormalTok{(MA\_m5, }\AttributeTok{col=}\StringTok{"red"}\NormalTok{, }\AttributeTok{lwd=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:amazn20LAG-1}Lag Plot que nos muestra la correlación entre 20 lags}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{06-ESTACIONARIEDAD_files/figure-latex/amazn20LAG-2.pdf}
\caption{\label{fig:amazn20LAG-2}Lag Plot que nos muestra la correlación entre 20 lags}
\end{figure}
